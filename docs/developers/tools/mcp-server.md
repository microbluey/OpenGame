# MCP servers with OpenGame

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with OpenGame.
...
OpenGame integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
OpenGame uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
OpenGame supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.opengame/mcp-oauth-tokens.json`
...
When OpenGame starts, it performs MCP server discovery through the following detailed process:
...
When OpenGame receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within OpenGame. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
## Managing MCP Servers with `opengame mcp`
...
The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.opengame/settings.json` or the project config `.opengame/settings.json` file.
...
```bash
# Basic syntax
opengame mcp add <name> <command> [args...]

# Example: Adding a local server
opengame mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
opengame mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
opengame mcp add --transport http <name> <url>

# Example: Adding an HTTP server
opengame mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
opengame mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
opengame mcp add --transport sse <name> <url>

# Example: Adding an SSE server
opengame mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
opengame mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"
```
