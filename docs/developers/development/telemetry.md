...
Learn how to enable and setup OpenTelemetry for OpenGame.
...
observability framework — OpenGame's observability system provides:
...
All telemetry behavior is controlled through your `.opengame/settings.json` file.
...
| `target`       | `QWEN_TELEMETRY_TARGET`        | `--telemetry-target <local\|opengame>`                   | Where to send telemetry data                      | `"opengame"`/`"local"` | `"local"`               |
...
1. Enable telemetry in your `.opengame/settings.json`:
   ```json
   {
     "telemetry": {
       "enabled": true,
   ...
   ```
2. Run OpenGame and send prompts.
...
1. Enable telemetry in your `.opengame/settings.json`:
   ```json
   {
     "telemetry": {
       ...
       "target": "local",
       "otlpEndpoint": "",
       "outfile": ".opengame/telemetry.log"
     }
   }
   ```
2. Run OpenGame and send prompts.
3. View logs and metrics in the specified file (e.g., `.opengame/telemetry.log`).
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.opengame/tmp/<projectHash>/otel/collector.log`
...
Logs are timestamped records of specific events. The following events are logged for OpenGame:
...
- `qwen-code.flash_fallback`: This event occurs when OpenGame switches to flash as fallback.
  - **Attributes**:
    - `auth_type`
...
Metrics are numerical measurements of behavior over time. The following metrics are collected for OpenGame (metric names remain `qwen-code.*` for compatibility):
