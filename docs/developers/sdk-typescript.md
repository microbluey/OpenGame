...
A minimum experimental TypeScript SDK for programmatic access to OpenGame.
...
- Node.js >= 20.0.0
- [OpenGame](https://github.com/leigest519/OpenGame) >= 0.4.0 (stable) installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the OpenGame executable. You should explicitly set the `pathToQwenExecutable` option to the full path of the `opengame` binary.
...
Creates a new query session with the OpenGame.
...
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the OpenGame executable. Supports multiple formats: `'opengame'` (native binary from PATH), `'/path/to/opengame'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `QWEN_CODE_CLI_PATH` env var, `~/.volta/bin/opengame`, `~/.npm-global/bin/opengame`, `/usr/local/bin/opengame`, `~/.local/bin/opengame`, `~/node_modules/.bin/opengame`, `~/.yarn/bin/opengame`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the OpenGame process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'qwen-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'qwen-oauth'` in SDK is not recommended as credentials are stored in `~/.opengame` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                                                     |
