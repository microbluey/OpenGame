...
This document explains how to run OpenGame inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up OpenGame:

```bash
npm install -g @opengame/opengame
```
...
By default, OpenGame uses a published sandbox image (configured in the CLI package) and will pull it as needed.
...
If you don’t set either, OpenGame uses the default image configured in the CLI package (for example `ghcr.io/leigest519/opengame:<version>`).
