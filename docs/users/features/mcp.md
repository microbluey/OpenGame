...
OpenGame can connect to external tools and data sources through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction). MCP servers give OpenGame access to your tools, databases, and APIs.
...
- By using `opengame mcp` commands (see [CLI reference](#qwen-mcp-cli))
...
3. Restart OpenGame in the same project (or start it if it wasn’t running yet), then ask the model to use tools from that server.

## Where configuration is stored (scopes)

Most users only need these two scopes:

- **Project scope (default)**: `.opengame/settings.json` in your project root
- **User scope**: `~/.opengame/settings.json` across all projects on your machine
...
```bash
opengame mcp add pythonTools -e DATABASE_URL=$DB_CONNECTION_STRING -e API_KEY=$EXTERNAL_API_KEY \
...
Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from OpenGame’s perspective).
...
- **Server shows “Disconnected” in `opengame mcp list`**: verify the URL/command is correct, then increase `timeout`.
- **Stdio server fails to start**: use an absolute `command` path, and double-check `cwd`/`env`.
- **Environment variables in JSON don’t resolve**: ensure they exist in the environment where OpenGame runs (shell vs GUI app environments can differ).
