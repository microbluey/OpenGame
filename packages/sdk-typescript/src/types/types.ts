 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @opengame/opengame-core.
 */
...
export interface QueryOptions {
  ...
  /**
   * Path to the OpenGame CLI executable or runtime specification.
   *
   * Supports multiple formats:
   * - 'opengame' -> native binary (auto-detected from PATH)
   ...
   *
   * If not provided, the SDK will use the bundled CLI shipped inside the SDK
   * package. The QWEN_CODE_CLI_PATH environment variable is still recognized
   * as an override for backward compatibility with the upstream agent runtime.
   *
   * The .ts files are only supported for debugging purposes.
   *
   * @example 'opengame'
   * @example '/usr/local/bin/opengame'
