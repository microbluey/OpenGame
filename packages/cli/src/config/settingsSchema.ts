...
const SETTINGS_SCHEMA = {
  ...
  general: {
    ...
    properties: {
      ...
      gitCoAuthor: {
        ...
        default: true,
        description:
          'Automatically add a Co-authored-by trailer to git commit messages when commits are made through OpenGame.',
        showInDialog: false,
      },
      checkpointing: {
        type: 'object',
  ...
  ui: {
    ...
    properties: {
      ...
      showStatusInTitle: {
        ...
        default: false,
        description:
          'Show OpenGame status and thoughts in the terminal window title',
        showInDialog: true,
      },
      hideTips: {
        type: 'boolean',
      ...
      hideProviderStatus: {
        type: 'boolean',
        label: 'Hide Provider Status Banner',
        category: 'UI',
        requiresRestart: false,
        default: false,
        description:
          'Hide the per-modality provider status line shown below the header at startup',
        showInDialog: true,
      },
      hideBanner: {
        type: 'boolean',
  ...
  openGame: {
    type: 'object',
    label: 'OpenGame Providers',
    category: 'Advanced',
    requiresRestart: true,
    default: {},
    description:
      "OpenGame's per-modality generative-AI provider configuration. " +
      'Each modality (reasoning / image / video / audio) can be pointed at ' +
      'Tongyi (Aliyun DashScope), Doubao (Volcengine ARK), or any ' +
      'OpenAI-compatible API. See docs/users/configuration/api-keys.md.',
    showInDialog: false,
    properties: {
      providers: {
        type: 'object',
        ...
        category: 'Advanced',
        requiresRestart: true,
        // Per-modality overrides. Any field left undefined falls back to the
        // matching OPENGAME_<MOD>_* environment variable, then to the legacy
        // upstream env vars (DASHSCOPE_API_KEY, IMAGE_MODEL_API_KEY, ...).
        default: undefined as
          | {
              reasoning?: {
          ...
          | undefined,
        description:
          'Per-modality provider configuration. Environment variables ' +
          '(OPENGAME_REASONING_*, OPENGAME_IMAGE_*, OPENGAME_VIDEO_*, ' +
          'OPENGAME_AUDIO_*) take precedence over these settings.',
        showInDialog: false,
      },
    },
  },
