...
import * as ServerConfig from '@opengame/opengame-core';
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/OPENGAME.md'],
      },
      {
        path: '/path/to/ext2',
        config: {
  ...
  it.skip('should correctly use mocked homedir for global path', async () => {
    const MOCK_GEMINI_DIR_LOCAL = path.join('/mock/home/user', '.opengame');
...
describe('mergeExcludeTools', () => {
  ...
  it('should exclude only shell tool in non-interactive mode when no approval flags are specified (auto-elevate to auto-edit)', async () => {
    // Headless mode auto-elevates to auto-edit so file edits work; only shell stays gated.
    ...
    expect(config.getExcludeTools()).toEqual([ShellTool.Name]);
  });
...
describe('Approval mode tool exclusion logic', () => {
  ...
  it('should auto-elevate to auto-edit in non-interactive mode with no approval mode specified', async () => {
    // Headless invocations like `opengame -p "..."` need to be able to write
    // files, otherwise the model is silently stripped of WriteFile/Edit and
    // claims it has no permission. Shell stays excluded until the user opts in.
    process.argv = ['node', 'script.js', '-p', 'test'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {};
    const extensions: Extension[] = [];
...
describe('loadCliConfig tool exclusions', () => {
  ...
  it('should exclude only shell in non-interactive mode without YOLO (headless auto-elevates to auto-edit)', async () => {
    process.stdin.isTTY = false;
    process.argv = ['node', 'script.js', '-p', 'test'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
  ...
  it('should still exclude all interactive tools in non-interactive mode when --approval-mode default is explicit', async () => {
    process.stdin.isTTY = false;
    process.argv = [
      'node',
      'script.js',
...
describe('loadCliConfig approval mode', () => {
  ...
  beforeEach(() => {
    ...
    // These tests cover the launch-without-prompt path, which should be
    // interactive. Force TTY = true so the headless auto-elevation to
    // AUTO_EDIT does not kick in and change DEFAULT-mode expectations.
    process.stdin.isTTY = true;
    vi.mocked(isWorkspaceTrusted).mockReturnValue(true);
  });
