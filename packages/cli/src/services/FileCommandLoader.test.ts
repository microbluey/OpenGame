...
import { Storage } from '@opengame/opengame-core';
...
vi.mock('@opengame/opengame-core', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('@opengame/opengame-core')>();
  return {
    ...original,
  ...
  describe('Extension Command Loading', () => {
    ...
    it('only loads commands from active extensions', async () => {
      const extensionDir1 = path.join(
        process.cwd(),
        '.opengame/extensions/active-ext',
      );
      const extensionDir2 = path.join(
        process.cwd(),
        '.opengame/extensions/inactive-ext',
      );
    ...
    it('handles missing extension commands directory gracefully', async () => {
      const extensionDir = path.join(
        process.cwd(),
        '.opengame/extensions/no-commands',
      );
    ...
    it('handles nested command structure in extensions', async () => {
      const extensionDir = path.join(process.cwd(), '.opengame/extensions/a');
