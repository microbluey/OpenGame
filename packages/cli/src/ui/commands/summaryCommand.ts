...
import { getProjectSummaryPrompt } from '@opengame/opengame-core';
...
export const summaryCommand: SlashCommand = {
  ...
  get description() {
    return t(
      'Generate a project summary and save it to .opengame/PROJECT_SUMMARY.md',
    );
  },
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    const saveSummaryToDisk = async (
      ...
      // Ensure .opengame directory exists
      const projectRoot = config.getProjectRoot();
      const qwenDir = path.join(projectRoot, '.opengame');
      try {
...
`;
      ...
      return {
        filePathForDisplay: '.opengame/PROJECT_SUMMARY.md',
