...
```bash
git clone https://github.com/leigest519/OpenGame.git # Or your fork's URL
cd OpenGame
```
...
> The `QWEN_SANDBOX` environment variable name is inherited from the upstream agent runtime and is still recognized by OpenGame.

To build both the `opengame` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the OpenGame application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the OpenGame folder, you can utilize `npm link path/to/OpenGame/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to expose the `opengame` command globally.
...
The integration tests are designed to validate the end-to-end functionality of OpenGame. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for OpenGame.
...
1.  **Start the OpenGame application in development mode:**
